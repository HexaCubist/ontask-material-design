...
class MaterialWidgetsForeignKeyTestModel(models.Model):
    u"""Secondary test model for ForeignKey."""
    item = models.IntegerField()

    class Meta(object):
        u"""Meta settings for MaterialWidgetsTestModel"""
        app_label = u'material_widgets_tests'


class MaterialWidgetsManyToManyTestModel(models.Model):
    u"""Secondary test model for ManyToManyField."""
    item = models.IntegerField()
...
class MaterialWidgetsTestModel(models.Model):
    u"""Test model containing all model fields with representative form fields.
    ...
    file_path_field = models.FilePathField(path=u'/demo/images', blank=True)
